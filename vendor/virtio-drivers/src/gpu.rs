...
impl<H: Hal, T: Transport> VirtIOGpu<'_, H, T> {
    fn build(mut transport: T) -> Result<alloc::boxed::Box<Self>> {
        ...
        Ok(alloc::boxed::Box::new(VirtIOGpu {
            transport,
    ...
    /// Create a new VirtIO-Gpu driver on heap to avoid large by-value return paths.
    pub fn new_boxed(transport: T) -> Result<alloc::boxed::Box<Self>> {
